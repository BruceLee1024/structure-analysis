...
import { SolverParams, AnalysisResult, Node, Element, Load, StructureType } from '../types';
...
const DiagramView = React.memo(({ 
    ...
    const toWorld = useCallback((px: number, py: number) => {
        const { width, height, scale, cx, cy } = transform;
        return {
            x: (px - width/2) / scale + cx,
            y: cy - (py - height/2) / scale
        };
    }, [transform]);
    ...
    // 1. Memoize Base Structure Layer
                ...
                {elements.map(el => {
                    ...
                    return (
                        <g key={el.id}>
                            ...
                            />
                            {/* Hide hinge circles if it's a Truss, because everything is hinged */}
                            {isEditor && !isTruss && (
                                <>
    ...
    // 3. Memoize Results Layer (Expensive paths)
    ...
    // 4. Calculate Active Hover Data (Cheap enough to run on render when activeLocation changes)
    const activeData = useMemo(() => {
        ...
        return {
            elementId: closestEl.id,
            t: closestT,
            globalX: n1.x + closestT * dx,
            globalY: n1.y + closestT * dy,
            moment: exactValues.moment,
            shear: exactValues.shear,
            axial: exactValues.axial,
            deflectionY: exactValues.deflectionY,
            n1, n2, length
        };
    ...
    const onDrop = useCallback((e: React.DragEvent) => {
        ...
        if ((type === 'point' || type === 'moment') && closestNode) {
            onAddLoad({
                ...
                type, nodeId: closestNode.id,
                magnitude: type === 'point' ? -10 : 10, direction: 'y'
            });
            return;
        }
        ...
        if (closestElement) {
            if (type === 'distributed') {
                onAddLoad({
                    id: Date.now().toString(),
                    type: 'distributed', elementId: closestElement.id,
                    magnitude: -5, direction: 'y'
                });
            } else {
                onAddLoad({
                    id: Date.now().toString(),
                    type, elementId: closestElement.id, location: hitT,
                    magnitude: type === 'point' ? -10 : 10, direction: 'y'
                });
            }
        }
    ...
    return (
        <div className="w-full h-full flex flex-col bg-slate-900 group">
            ...
            <div className="flex-1 relative min-h-0 overflow-hidden">
                ...
                >
                    <defs>
                        <marker id="arrowhead-force" markerWidth="8" markerHeight="6" refX="0" refY="3" orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#f59e0b" /></marker>
                    ...
                    {activeData && (() => {
                        ...
                        return (
                            <g>
                                {!isEditor && Math.abs(val) > 1 && 
                                    <line x1={px.x} y1={px.y} x2={tipX} y2={tipY} stroke="white" strokeDasharray="3 3" strokeOpacity="0.6" />
                                }
                                <circle 
                                    cx={px.x} cy={px.y} r={isEditor ? "5" : "3"} 
                                    fill={isEditor ? "#fbbf24" : "white"} fillOpacity={isEditor ? "1" : "0.8"} 
                                    stroke={isEditor ? "white" : "none"} strokeWidth={isEditor ? "2" : "0"} 
                                />
                                {!isEditor && 
                                    <circle cx={tipX} cy={tipY} r="5" fill="#fbbf24" stroke="white" strokeWidth="2"/>
                                }
                                {isEditor && (
...
const StructureVisualizer: React.FC<StructureVisualizerProps> = ({ params, nodes, elements, results, loads, onAddLoad }) => {
  ...
  // 1. Calculate Geometry Bounds & Scale
  ...
  // 2. Pre-calculate Max Values for Scaling
